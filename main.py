...
from datasets_change import Data
import cv2
...
def test(checkpoint_path, dataloader, model, device, output_dir, args,lis_img):
    ...
    for i, data in enumerate(dataloader):
        # if args.cuda:
        #     data = data.cuda()
        # data_information = resize_keep_ratio(data, (720, 1280))
        data1 = data[0]
        with torch.no_grad():
        ...
        left = int(data[3])
        right = int(data[4])
        fuse = fuse[top:720-bottom,left:1280-right]
        # fuse = cv2.resize(fuse,(int(data[5]),int(data[6])))
        ...
        name = lis_img[i].split("/")[-1][:-4]
        # print(name)
        cv2.imencode(".png", 255 - fuse * 255)[1].tofile(os.path.join(save_dir, '%s.png' % name))
        # all_t += time.time() - tm
    # print(all_t)
...
def main(args,lis_img,save_path):
    """Main function."""

    with open("dealing/extract.lst", "w") as f1:
    ...
    #                           mean_bgr=args.mean_pixel_values[0:3] if len(
    #                               args.mean_pixel_values) == 4 else args.mean_pixel_values,
    #                           test_list="BIPED/edges/test_rgb.lst", arg=args
    #                           )
    ...
    # test_img = Data("data/BIPED/edges/imgs/test/", "test_rgb.lst",crop_size=True,mean_bgr=mean_bgr)
    test_img = Data("dealing", "extract.lst", crop_size=True, mean_bgr=mean_bgr)
    # test_img = Data("data/", "test_cus.lst", crop_size=True, mean_bgr=mean_bgr)
    ...
    #     output_dir = os.path.join(args.res_dir, args.train_data+"2"+ args.test_data)
        ...
        #     testPich('checkpoints\BIPED\\10_model.pth', dataloader_val, model, device, output_dir, args)
        # else:
    test("checkpoints\BIPED\\10_model.pth", dataloader_val, model, device, save_path, args,lis_img)
